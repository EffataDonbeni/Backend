...
router.post(
  ...
  [
    ...
    [
      ...
      check("title", "Title must be less than 200 characters").isLength({
        max: 200,
      }),
      check("excerpt", "Excerpt is required").not().isEmpty(),
      check("excerpt", "Excerpt must be less than 300 characters").isLength({
        max: 300,
      }),
...
router.put(
  ...
  [
    ...
    adminAuth,
    [
      check("title", "Title must be less than 200 characters")
        .optional()
        .isLength({ max: 200 }),
      ...
      check("category", "Invalid category")
        .optional()
        .isIn(["design", "development", "ui-ux", "tutorial", "tips"]),
    ],
  ],
...
router.post(
  ...
  [
    ...
    [
      ...
      check("content", "Comment must be less than 500 characters").isLength({
        max: 500,
      }),
    ],
  ],
...
// Flag a comment (user)
router.post("/:id/comment/:comment_id/flag", auth, blogController.flagComment);
// Moderate a flagged comment (admin)
router.patch(
  "/:id/comment/:comment_id/moderate",
  auth,
  adminAuth,
...
router.get(
  ...
  auth,
  adminAuth,
  blogController.getAllCommentsForAdmin
);

// Admin: Get blog stats
router.get("/blogs/admin/stats", auth, adminAuth, blogController.getBlogStats);
